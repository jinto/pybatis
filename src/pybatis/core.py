...
class SqlSession(ABC):
    ...
    @abstractmethod
    async def execute(self, sql: str, parameters: Optional[Dict[str, Any]] = None) -> Any:
        """SQL을 실행합니다."""
        pass

    @abstractmethod
    async def fetch_one(self, sql: str, parameters: Optional[Dict[str, Any]] = None) -> Optional[Dict[str, Any]]:
        """하나의 레코드를 조회합니다."""
        pass

    @abstractmethod
    async def fetch_all(self, sql: str, parameters: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        """모든 레코드를 조회합니다."""
        pass
...
class PyBatisMapper:
    ...
    async def select_one(self, sql: str, parameters: Optional[Dict[str, Any]] = None,
                        result_type: Optional[Type[T]] = None) -> Optional[T]:
    ...
    async def select_list(self, sql: str, parameters: Optional[Dict[str, Any]] = None,
                         result_type: Optional[Type[T]] = None) -> List[T]:
    ...
    async def insert(self, sql: str, parameters: Optional[Dict[str, Any]] = None) -> Any:
        """INSERT 문을 실행합니다."""
        return await self.session.execute(sql, parameters)

    async def update(self, sql: str, parameters: Optional[Dict[str, Any]] = None) -> Any:
        """UPDATE 문을 실행합니다."""
        return await self.session.execute(sql, parameters)

    async def delete(self, sql: str, parameters: Optional[Dict[str, Any]] = None) -> Any:
        """DELETE 문을 실행합니다."""
        return await self.session.execute(sql, parameters)


def sql_query(sql: str, result_type: Optional[Type[T]] = None):
...
def sql_select(sql: str, result_type: Optional[Type[T]] = None):
...
async def get_sql_session() -> SqlSession:
    ...
    raise NotImplementedError("이 함수는 구체적인 데이터베이스 구현에서 오버라이드되어야 합니다.")
